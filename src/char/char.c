...
int char_mmo_char_tosql(int char_id, struct mmo_charstatus* p)
{
	...
	if (
		...
		(p->uniqueitem_counter != cp->uniqueitem_counter) || (p->hotkey_rowshift != cp->hotkey_rowshift) || (p->queue_delay != cp->queue_delay)
	) {
		//Save status
		unsigned int opt = 0;
		...
		if( SQL_ERROR == SQL->Query(inter->sql_handle, "UPDATE `%s` SET `base_level`='%d', `job_level`='%d',"
			...
			"`hotkey_rowshift`='%d', `queue_delay_timer`='%d'"
			...
			p->hotkey_rowshift, p->queue_delay,
	...
	if (save_status[0]!='\0' && save_log)
		ShowInfo("Salvando personagem %d - %s:%s.\n", char_id, p->name, save_status);
	if (!errors)
...
int char_memitemdata_to_sql(const struct item items[], int max, int id, int tableswitch)
{
	...
	default:
		ShowError("Nome de tabela invalido!\n");
		return 1;
	}
...
int char_mmo_char_fromsql(int fd, int char_id, struct mmo_charstatus* p, bool load_everything)
{
	...
	if (SQL_ERROR == SQL->StmtPrepare(stmt, "SELECT "
		...
		"`char_opt`,`font`,`uniqueitem_counter`,`sex`,`hotkey_rowshift`,`queue_delay_timer`"
	 ...
	 || SQL_ERROR == SQL->StmtBindColumn(stmt, 59, SQLDT_INT,    &p->queue_delay, 0, NULL, NULL)
	) {
		SqlStmt_ShowDebug(stmt);
		SQL->StmtFree(stmt);
		return 0;
 ...
 *  char_id: Success
 **/
int char_make_new_char_sql(struct char_session_data* sd, char* name_, int str, int agi, int vit, int int_, int dex, int luk, int slot, int hair_color, int hair_style, int race, uint8 sex) {
	...
	switch(race) {
		...
		case RACE_DORAM:
			...
			break;
		default:
			ShowWarning("char_make_new_char_sql: Pacote de criacao de personagem com raca (%d) invalida detectado na conta: %d.\n", race, sd->account_id);
			return -2;
	}
	...
	ShowInfo("Personagem criado: conta: %d, char id: %d, slot: %d, nome: %s, sexo: %c\n", sd->account_id, char_id, slot, name, sex);
	return char_id;
}
...
void char_parse_frommap_request_stats_report(int fd)
{
	...
	if ((sfd = sockt->make_connection(sockt->host2ip("stats.brathena.org"),(uint16)25427,&opt) ) == -1) {
...
void char_parse_char_create_new_char(int fd, struct char_session_data* sd)
{
	...
	if( !char_new ) {
		...
		result = chr->make_new_char_sql(sd, (char*)RFIFOP(fd,2), 1, 1, 1, 1, 1, 1, RFIFOB(fd,26),RFIFOW(fd,27),RFIFOW(fd,29), RFIFOW(fd,31), RFIFOB(fd, 35));
#elif PACKETVER >= 20120307
		result = chr->make_new_char_sql(sd, (char*)RFIFOP(fd,2), 1, 1, 1, 1, 1, 1, RFIFOB(fd,26),RFIFOW(fd,27),RFIFOW(fd,29), RACE_HUMAN, 'U');
#else
		result = chr->make_new_char_sql(sd, (char*)RFIFOP(fd,2), RFIFOB(fd,26), RFIFOB(fd,27), RFIFOB(fd,28), RFIFOB(fd,29), RFIFOB(fd,30), RFIFOB(fd,31), RFIFOB(fd,32), RFIFOW(fd,33), RFIFOW(fd,35), RACE_HUMAN, 'U');
#endif
	}
