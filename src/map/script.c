...
void script_load_translations(void) {
	...
	const char *config_filename = "lang/lang.conf"; // FIXME hardcoded name
	config_setting_t *translations = NULL;
	int i, size, language = 0;
	uint32 total = 0;
	uint8 lang_id = 0, k;
	...
	ShowConf("Leitura de '"CL_WHITE"%d"CL_RESET"' idiomas em '"CL_WHITE"%s"CL_RESET"'.\n", language, config_filename);
}
...
BUILDIN(monster)
{
	...
	if (sd && strcmp(mapn, "this") == 0)
		...
		for(i = 0; i < amount; i++) {
			mob_id = mob->once_spawn(sd, m, x, y, str, class_, 1, event, size, ai);
...
BUILDIN(areamonster) {
	...
	if (sd && strcmp(mapn, "this") == 0)
		...
		for(i = 0; i < amount; i++) { //not optimised
			mob_id = mob->once_spawn_area(sd, m, x0, y0, x1, y1, str, class_, 1, event, size, ai);
 ...
 * Adicionado suporte para cores RGB. [Shiraz]
 *------------------------------------------*/
BUILDIN(dispbottom)
{
...
BUILDIN(duplicatenpc)
{
	...
	nd_target = npc->create_npc(SCRIPT, tmapid, tx, ty, 0, tclass_);
...
BUILDIN(getunitdata)
{
	...
	switch (bl->type) {
		case BL_MOB:  md = map->id2md(bl->id); break;
		case BL_HOM:  hd = map->id2hd(bl->id); break;
