...
* Funções referentes à BG (battleground)                                     *
******************************************************************************
	...
	if (--bgd->count != 0) {
		...
		switch (flag) {
			...
			case BGTL_QUIT:
				...
				break;
			case BGTL_LEFT:
				sprintf(output, "Servidor : %s saiu do campo de batalha...", sd->status.name);
				break;
			case BGTL_AFK:
		...
		if( (arenas = libconfig->setting_get_member(settings, "arenas")) != NULL ) {
			...
			for(i = 0; i < arena_count; i++) {
				...
				if( !libconfig->setting_lookup_string(arena, "name", &aName) ) {
					ShowError("bg_config_read: falha ao encontrar 'name' para a arena #%d\n",i);
					continue;
				}

				if( !libconfig->setting_lookup_string(arena, "event", &aEvent) ) {
					ShowError("bg_config_read: falha ao encontrar 'event' para a arena #%d\n",i);
					continue;
				}
				...
				if( !(reward = libconfig->setting_get_member(arena, "reward")) ) {
					ShowError("bg_config_read: falha ao encontrar 'reward' para a arena '%s'/#%d\n",aName,i);
					continue;
				}
				...
				if( prizeWin < 0 ) {
					ShowWarning("bg_config_read: valor %d invalido para arena '%s' reward:win\n",prizeWin,aName);
					prizeWin = 0;
				}
				if( prizeLoss < 0 ) {
					ShowWarning("bg_config_read: valor %d invalido para arena '%s' reward:loss\n",prizeLoss,aName);
					prizeLoss = 0;
				}
				if( prizeDraw < 0 ) {
					ShowWarning("bg_config_read: valor %d invalido para arena '%s' reward:draw\n",prizeDraw,aName);
					prizeDraw = 0;
				}
				...
				if( minPlayers < 0 ) {
					ShowWarning("bg_config_read: valor %d invalido para arena '%s' minPlayers\n",minPlayers,aName);
					minPlayers = 0;
				}
				if( maxPlayers > MAX_BG_MEMBERS * 2 ) {
					ShowWarning("bg_config_read: valor %d invalido para arena '%s' maxPlayers, modifique #define MAX_BG_MEMBERS\n",maxPlayers,aName);
					maxPlayers = 0;
				}
				if( minTeamPlayers < 0 ) {
					ShowWarning("bg_config_read: valor %d invalido para arena '%s' minTeamPlayers\n",minTeamPlayers,aName);
					minTeamPlayers = 0;
				}

				if( !libconfig->setting_lookup_string(arena, "delay_var", &aDelayVar) ) {
					ShowError("bg_config_read: falha ao encontrar 'delay_var' para a arena '%s'/#%d\n",aName,i);
					continue;
				}

				if( !libconfig->setting_lookup_string(arena, "allowedTypes", &aTeamTypes) ) {
					ShowError("bg_config_read: falha ao encontrar 'allowedTypes' para a arena '%s'/#%d\n",aName,i);
					continue;
				}
				...
				if( maxDuration < 0 ) {
					ShowWarning("bg_config_read: valor %d invalido para arena '%s' maxDuration\n",maxDuration,aName);
					maxDuration = 30;
				}
				...
				if( fillup_duration < 20 ) {
					ShowWarning("bg_config_read: valor %d invalido para arena '%s' fillDuration, minimo tem de ser 20, definindo para 20.\n",fillup_duration,aName);
					fillup_duration = 20;
				}

				if( pregame_duration < 20 ) {
					ShowWarning("bg_config_read: valor %d invalido para arena '%s' pGameDuration, minimo tem de ser 20, definindo para 20.\n",pregame_duration,aName);
					pregame_duration = 20;
				}
...
enum BATTLEGROUNDS_QUEUE_ACK bg_canqueue(struct map_session_data *sd, struct bg_arena *arena, enum bg_queue_types type) {
	...
	switch(type) {
		...
		case BGQT_INDIVIDUAL:/* already did */
			break;
		default:
			ShowDebug("bg_canqueue: tipo desconhecido %d\n",type);
			return BGQA_DUPLICATE_REQUEST;
	}
	return BGQA_SUCCESS;
}
