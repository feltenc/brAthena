...
/// 09e5 <index>.W <amount>.W <GID>.L <Date>.L <zeny>.L (ZC_DELETEITEM_FROM_MCSTORE2).
 void clif_vendingreport(struct map_session_data* sd, int index, int amount, uint32 char_id, int zeny) {
 #if PACKETVER < 20141016		// TODO : not sure for client date [Napster][Rathena]
 	const int cmd = 0x137;
 #else
 	const int cmd = 0x9e5;
 #endif
	...
	WFIFOW(fd,4) = amount;
 	...
 	WFIFOL(fd,10) = (int)time(NULL);	// Data da transacao
...
void clif_parse_OneClick_ItemIdentify(int fd, struct map_session_data *sd)
{
	...
	if ((n = pc->have_magnifier(sd) ) != INDEX_NOT_FOUND &&
		pc->delitem(sd, n, 1, 0, DELITEM_NORMAL) == 0)
...
void clif_parse_Auction_register(int fd, struct map_session_data *sd)
{
	...
	else
	{
		...
		pc->delitem(sd, sd->auction.index, sd->auction.amount, 1, DELITEM_SOLD);
		sd->auction.amount = 0;
...
void clif_buyingstore_update_item(struct map_session_data* sd, unsigned short nameid, unsigned short amount, uint32 char_id, int zeny) {
	...
	WFIFOW(fd,0) = cmd;
	WFIFOW(fd,2) = nameid;
	WFIFOW(fd,4) = amount;  // Quantidade de nameID recebida
 ...
 WFIFOL(fd,14+offset) = (int)time(NULL);		// Data da transacao
 #endif
			...
			if( sd->kafraPoints < kafra_pay ) {
				...
				for (k = 0; k < qty; k += get_count) {
					...
					if(result == CSBR_SUCCESS) /* Add to cashshop Log - [GreenStage]*/
						logs->cash_buy_sql(sd,"Bot�o de Cash","",&item_tmp,get_count,clif->cs.data[tab][j]->price);
