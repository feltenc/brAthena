...
******************************************************************************
* src/map/battle.c                                                           *
* Cálculo de fórmulas de batalha do emulador                                 *
******************************************************************************
...
int64 battle_attr_fix(struct block_list *src, struct block_list *target, int64 damage,int atk_elem,int def_type, int def_lv)
{
	...
	if (def_type < ELE_NEUTRAL || def_type >= ELE_MAX ||
		def_lv < 1 || def_lv > 4) {
		ShowError("battle_attr_fix: Tipo de attr desconhecido: atk=%d def_type=%d def_lv=%d\n",atk_elem,def_type,def_lv);
		return damage;
	}
		...
		case BF_WEAPON:
			...
			if (tsd) {
				...
				if( src->type == BL_MOB && //Only affected from mob
					...
					(battle->check_undead(sstatus->race,sstatus->def_ele) || sstatus->race==RC_UNDEAD) )	//Undead attacker
		...
		case BF_WEAPON:
			switch( skill_id )
			{
				...
				case GC_DARKCROW:
					...
					break;
				case RL_QD_SHOT:
					skillratio += -100 + (max(pc->checkskill(sd,GS_CHAINACTION),1) * status_get_dex(src) / 5); //(custom)
					break;
				...
				case RL_FIREDANCE:
					skillratio += -100 + 100 * skill_lv;
					skillratio += (skillratio * status->get_lv(src)) / 300; //(custom)
					break;
			...
			default: {
				...
				if (nk&NK_SPLASHSPLIT) { // Divide MATK in case of multiple targets skill
					if(mflag>0)
						...
						ShowError("0 inimigos alvo de %d:%s, divisao por 0 evitada!\n", skill_id, skill->get_name(skill_id));
				}
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int mflag) {
	...
	case RL_B_TRAP:
		...
		md.damage = ((200 + sstatus->dex) * (int64)skill_lv * 10) + sstatus->hp; // (custom)
		break;
	case KO_HAPPOKUNAI:
		{
...
struct Damage battle_calc_weapon_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int wflag)
{
	...
	if (sd && !skill_id) {
		...
		else if( ((sd->weapontype1 == W_REVOLVER && (skill_lv = pc->checkskill(sd,GS_CHAINACTION)) > 0) //Normal Chain Action effect
...
#define GET_NORMAL_ATTACK2( f , s ) ( wd.damage2 = battle->calc_base_damage(src, target, s, skill_lv, nk, n_ele, s_ele, s_ele_, EQI_HAND_L, (f), wd.flag) )
		...
		if( sd ) {
			...
			if( (i=pc->checkskill(sd,AB_EUCHARISTICA)) > 0 &&
				(tstatus->race == RC_DEMON || tstatus->def_ele == ELE_DARK) )
				ATK_ADDRATE(-i);
			if (skill_id != PA_SACRIFICE && skill_id != MO_INVESTIGATE && skill_id != CR_GRANDCROSS && skill_id != NPC_GRANDDARKNESS && skill_id != PA_SHIELDCHAIN && skill_id != RL_MASS_SPIRAL && !flag.cri) {
		...
		if (skill_id == NPC_EARTHQUAKE) {
			...
			if ( wflag>0 )
				...
				ShowError("0 range por %d:%s, divisao por 0 evitada!\n", skill_id, skill->get_name(skill_id));
		}
#endif
...
enum damage_lv battle_weapon_attack(struct block_list* src, struct block_list* target, int64 tick, int flag) {
	...
	if( sc && sc->count ) {
		...
		if( sd && sc->data[SC_FEARBREEZE] && sc->data[SC_FEARBREEZE]->val4 > 0 && sd->status.inventory[sd->equip_index[EQI_AMMO]].amount >= sc->data[SC_FEARBREEZE]->val4 && battle_config.arrow_decrement){
			...
			pc->delitem(sd,sd->equip_index[EQI_AMMO],sc->data[SC_FEARBREEZE]->val4,0,DELITEM_SKILLUSE);
...
// eAthena additions
	...
	*/
	...
	{ "vending_chat_block_range_check_cells",		&battle_config.vending_chat_block_range_check_cells,    0,      0,     10,				},
	{ "vending_chat_block_range_count_players",		&battle_config.vending_chat_block_range_count_players,  5,      0,     30,				},
	// Block Refine - [Orce]
