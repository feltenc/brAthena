...
******************************************************************************
* src/map/status.c                                                           *
* Fun��es respons�veis pelo c�lculo de status e atributos                    *
******************************************************************************
...
#define set_sc_with_vfx(skill, sc, icon, flag) do { set_sc((skill), (sc), (icon), (flag)); if((icon) < SI_MAX) status->dbs->RelevantBLTypes[(icon)] |= BL_SCEFFECT; } while(0)
	...
	**/
	...
	status->dbs->DisplayType[SC_C_MARKER]			 = true;
	status->dbs->DisplayType[SC_ANTI_M_BLAST]		 = true;
...
#define SCDEF_LVL_DIFF(bl, src, maxlv, factor) ( ( SCDEF_LVL_CAP((bl), (maxlv)) - SCDEF_LVL_CAP((src), (maxlv)) ) * (factor) )
	...
	case SC_NETHERWORLD:
		...
		break;
	case SC_B_TRAP:
		tick_def = (sd ? sd->status.str : status->get_base_status(bl)->str) * 50; // (custom)
		break;
	default:
...
int status_change_start(struct block_list *src, struct block_list *bl, enum sc_type type, int rate, int val1, int val2, int val3, int val4, int tick, int flag) {
	...
	switch (type) {
		...
		case SC_HEAT_BARREL:
			...
			if (sc->data[SC_P_ALTER] || sc->data[SC_GS_MADNESSCANCEL])
				return 0;
			break;
		case SC_P_ALTER:
		...
		case SC_H_MINE:
			{
				...
				if (sce->val3 || status_isdead(bl) || !(caster = map->id2sd(sce->val2)))
					break;
				if (!itemdb->exists(skill->get_itemid(RL_H_MINE,0)))
					break;
				memset(&it, 0, sizeof(it));
				it.nameid = skill->get_itemid(RL_H_MINE,0);
				it.amount = max(skill->get_itemqty(RL_H_MINE,0),1);
				it.identify = 1;
				map->addflooritem(bl, &it, it.amount, bl->m,bl->x, bl->y, caster->status.char_id, 0, 0, 4);
			}
			break;
		case SC_HEAT_BARREL:
			if (sd)
...
#define sc_timer_next(t,f,i,d) do { \
	...
	switch(type) {
		...
		case SC_C_MARKER:
			if( --(sce->val4) >= 0 ) {
				TBL_PC *caster = map->id2sd(sce->val2);
...
int status_readdb(void)
{
	...
	sv_readsqldb(get_database_name(55), 2, SC_MAX, status->readdb_scconfig);
	status->read_job_db();
